...
Partial Class iPxAdmin_iPxAPTransaction
    ...
    Sub UserAcces()
        ...
        sSQL += "(select 'Y' from iPxAcct_profile_client_useraccess_dtl as x where x.funtionid='10040'and x.active='Y' and x.businessid=a.businessid and x.usercode=a.usercode) as AddAR, "
        sSQL += "(select 'Y' from iPxAcct_profile_client_useraccess_dtl as x where x.funtionid='10043'and x.active='Y' and x.businessid=a.businessid and x.usercode=a.usercode) as import "
    ...
    Sub ListARHeader()
        ...
        sSQL += "(select sum(c.amountcr) from iPxAcctAP_Transaction as c where c.TransID=b.TransID) as amountTrans, "
        sSQL += "(select COUNT(PVno) from iPxAcctAP_Transaction as d where d.TransID=b.TransID and PVno<>'') as totInv, "
        sSQL += "(select 'Y' from iPxAcct_profile_client_useraccess_dtl as x where x.businessid=b.businessid and x.usercode='" & Session("sUserCode") & "' and x.funtionid='10044' and x.active='Y') as posting, "
        sSQL += "(select 'Y' from iPxAcct_profile_client_useraccess_dtl as x where x.businessid=b.businessid and x.usercode='" & Session("sUserCode") & "' and x.funtionid='10042' and x.active='Y') as deleteAR "
        sSQL += "FROM iPx_profile_client AS a RIGHT OUTER JOIN iPxAcctAP_TransHdr AS b ON a.businessid = b.InvLink COLLATE Latin1_General_CI_AS "
    ...
    Public Function Posting(ByVal businessid As String, ByVal TransID As String) As Boolean
        ...
        End If
        oSQLCmd = New SqlCommand(sSQL, oCnct)
        sSQL = "SELECT * FROM iPxAcctAP_TransHdr where businessid ='" & businessid & "' and TransID='" & TransID & "' "
        oSQLCmd.CommandText = sSQL
        oSQLReader = oSQLCmd.ExecuteReader
    ...
    Public Function saveGLHeader(ByVal GLid As String, ByVal businessid As String, ByVal TransID As String, ByVal GLDate As String, ByVal GLDesc As String) As Boolean
        ...
        sSQL1 += "'O','G4','" & TransID & "','" & Replace(GLDesc, "'", "''") & "') "
    ...
    Public Function GLDetail(ByVal businessid As String, ByVal TransID As String, ByVal GLid As String) As Boolean
        ...
        sSQL2 = "SELECT a.RecID, b.VendorID, b.CoyName, b.CoaLink, c.Description, c.Coa, a.POno, a.amountcr "
        ...
        sSQL2 += "INNER JOIN iPxAcctAP_Cfg_Transactiontype as c ON a.businessid COLLATE SQL_Latin1_General_CP1_CI_AS = c.businessid AND a.transactiontype = c.TransactionType "
        ...
        While oSQLReader2.Read
            saveGLDetailCredit(businessid, GLid, oSQLReader2.Item("Coa"), oSQLReader2.Item("CoyName") + " - " + oSQLReader2.Item("POno"), TransID, oSQLReader2.Item("amountcr"))
            saveGLDetailDebit(businessid, GLid, oSQLReader2.Item("CoaLink"), oSQLReader2.Item("CoyName") + " - " + oSQLReader2.Item("POno"), TransID, oSQLReader2.Item("amountcr"))
        End While
        oCnct2.Close()
    ...
    Sub EditPosting()
        ...
        End If
        oSQLCmd = New SqlCommand(sSQL, oCnct)
        sSQL = "UPDATE iPxAcctAP_TransHdr SET Status='P'"
    ...
    Sub deleteStatus()
        ...
        End If
        oSQLCmd = New SqlCommand(sSQL, oCnct)
        sSQL = "UPDATE iPxAcctAP_TransHdr SET Status='X'"
        sSQL = sSQL & "WHERE businessid ='" & Session("sBusinessID") & "' and TransID='" & Session("sTransID") & "'"
    ...
    Sub cekARDetailCL()
        ...
        sSQL += "LEFT JOIN dbo.iPxAcctAP_Cfg_FOmapping as c ON b.businessid = c.Folink AND b.companycode = c.profilecode "
        ...
        If oSQLReader.HasRows Then
            ...
            saveGrpARCL()
            ListARHeader()
            cIpx.saveLog(Session("sBusinessID"), ARCodeImport, Transdate, "AP", "Import", "Import AP dari " & dlFOLinkImport.SelectedItem.ToString, dlFOLinkImport.SelectedValue, Session("sUserCode"))
    ...
    Sub cekARDetailCC()
        ...
        sSQL += "FROM dbo.iPxPMS_postingrecord as a "
        sSQL += "INNER JOIN dbo.iPxPMS_reservation as b ON a.businessid = b.businessid AND a.rsvpno = b.rsvpno AND a.rsvpid = b.id "
        sSQL += "LEFT OUTER JOIN dbo.iPxAcctAP_Cfg_FOmapping as c ON a.cardtype = c.profilecode "
    ...
    Public Function saveARDetail(ByVal businessid As String, ByVal TransID As String, ByVal TransDate As String, ByVal CustomerID As String, ByVal Type As String, ByVal InvoiceNo As String, ByVal voucherNo As String, ByVal fulioNo As String, ByVal rsvpNo As String, ByVal GuestName As String, ByVal RoomNo As String, ByVal arrival As String, ByVal departure As String, ByVal notes As String, ByVal AmountDr As String, ByVal AmountCr As String, ByVal AmountDrFr As String, ByVal AmountCrFr As String, ByVal currency As String, ByVal businessidfo As String) As Boolean
        ...
        sSQL = "INSERT INTO iPxAcctAP_Transaction(businessid,TransID,TransDate,CustomerID,transactiontype,invoiceno,voucherno, "
    ...
    Sub cekCustDetailAR()
        ...
        sSQL += "(select COUNT(e.CustomerID) from iPxAcctAP_Transaction as e where e.TransID=b.TransID and e.CustomerID='') as TotCust, "
        sSQL += "(select COUNT(f.transactiontype) from iPxAcctAP_Transaction as f where f.TransID=b.TransID and f.transactiontype='') as totPay "
        sSQL += "FROM iPx_profile_client AS a RIGHT OUTER JOIN iPxAcctAP_TransHdr AS b ON a.businessid = b.FoLink COLLATE Latin1_General_CI_AS "
    ...
    Sub cekCloseGrid()
        ...
        Else
            ...
            cIpx.saveLog(Session("sBusinessID"), Session("sTransID"), TransDateLog, "AP TR", "Posting", "Posting AP Transaction", "", Session("sUserCode"))
        End If
        oCnct4.Close()
    End Sub
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Me.IsPostBack Then
            If cIpx.getAccessUser(Session("sBusinessID"), Session("sUserCode"), "AP Transaction") <> True Then
            ...
            If Session("sDateWorkAP") = "" Then
                tbDate.Text = Format(Now, "MM-yyyy")
            Else
                tbDate.Text = Session("sDateWorkAP")
            End If
            ListARHeader()
        Else
            ListARHeader()
    ...
    Protected Sub TransdateWork(ByVal sender As Object, ByVal e As EventArgs)
        Session("sDateWorkAP") = tbDate.Text
        Session("sQueryTicket") = ""
    End Sub
    ...
    Protected Sub lbAddAR_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbAddAR.Click
        ...
        Response.Redirect("iPxAPInputTransaction.aspx")
    End Sub
    ...
    Protected Sub gvARTrans_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvARTrans.RowCommand
        ...
        ElseIf e.CommandName = "getPost" Then
            ...
            sSQL += "(select COUNT(e.VendorID) from iPxAcctAP_Transaction as e where e.TransID=b.TransID and e.VendorID='') as TotCust, "
            sSQL += "(select COUNT(f.transactiontype) from iPxAcctAP_Transaction as f where f.TransID=b.TransID and f.transactiontype='') as totPay, "
            ...
            sSQL += "(SELECT COUNT(c.Coa) FROM iPxAcctAP_Transaction as a "
            sSQL += "INNER JOIN iPxAcctAP_Cfg_Transactiontype as c ON a.businessid COLLATE SQL_Latin1_General_CP1_CI_AS = c.businessid "
            ...
            sSQL += "FROM iPxAcctAP_TransHdr AS b "
            ...
            ElseIf totCust <> "0" Then
                ScriptManager.RegisterStartupScript(Me, Me.GetType(), "alertMessage", "alert('Posting process failed, Vendor id is empty !');document.getElementById('Buttonx').click()", True)
        ...
        ElseIf e.CommandName = "getPrint" Then
            Dim cIpx As New iPxClass
            Session("sReport") = "APTrans"
            Session("sTransID") = e.CommandArgument
            'tampilFO()
            'Session("filename") = "dckCashierSummary_Form.rpt"
            Session("sMapPath") = "~/iPxReportFile/AP_trans_dck.rpt"
    ...
    Protected Sub lbDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbDelete.Click
        ...
        Else
            ...
            deleteStatus()
            cIpx.saveLog(Session("sBusinessID"), Session("sTransID"), Transdate, "AP TR", "Delete", tbReason.Text, "", Session("sUserCode"))
            ScriptManager.RegisterStartupScript(Me, Me.GetType(), "hideModalDeleteTrans", "hideModalDeleteTrans()", True)
        End If
    End Sub
